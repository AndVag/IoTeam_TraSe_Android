...
//        camera.setTarget(new GeoCoordinates(52.520798, 13.409408));
        camera.setTarget(new GeoCoordinates(59.95, 30.32));
    ...
    private void showRouteDetails(Route route) {
        ...
        String routeDetails =
                "Время в пути: \t " + formatTime(estimatedTravelTimeInSeconds)
                        + ", \nДлина пути: \t " + formatLength(lengthInMeters)
                        + ", \nПешком: \t\t " + formatLength(lengthInMeters/7)
                        + ", \nКилокалорий: \t " + formatEnergi(lengthInMeters)
                        + ", \nБаллов: \t " + Math.round(lengthInMeters * 1.6);

//        showDialog("Route Details", routeDetails);
        showDialog("Параметры маршрутв", routeDetails);
    }
    ...
    private String formatEnergi(int meters) {
        int remainingMeters = meters;

        return String.format(Locale.getDefault(), "%02d", remainingMeters);
    }
